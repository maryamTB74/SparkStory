...
factually wrong -- so it must be representable. This is non-obvious rule 14 one
level up from the review loops: there the empty list is a loop's stop signal, here
it is a legitimate result. Requiring one fact would not produce a schema error, it
...
class GroundedFact(BaseModel):
    ...
    # Provenance, and the reason it exists is one session ahead: once research is
    # its own MCP tool, a client threads this object back to us and nothing else
    # distinguishes a fact we supplied from one a client invented. An id we can
    # look up turns that into a check. See spec section 4.
    chunk_id: str = Field(
        min_length=1,
        max_length=120,
        description=(
...
class StoryGrounding(BaseModel):
    ...
    # (finding D: 6 beats offered, 6 beats used, every time), so three facts is
    # three facts.
